...
public class MipGenerator
{
    ...
    public MipGenerator()
    {
        ...
        blitMaterial = new Material(Shader.Find("MipGenerator/Blit"));
    }

    public void GenerateCustomMips(RenderTexture target, Material mipMaterial)
    {
        int size = target.width;
        int numMips = (int) (Math.Log(target.width) / Math.Log(2)) + 1;
        RenderTexture[] mipTextures = new RenderTexture[numMips];
        for (int i = 0; i < numMips; i++)
        {
            int mipRes = (int) (size / Math.Pow(2, i));

            RenderTexture rt = RenderTexture.GetTemporary(target.descriptor);
            rt.width = mipRes;
            rt.height = mipRes;
            rt.depth = 0;
        ...
        for (int i = 0; i < numMips; i++)
        {
            RenderTexture.ReleaseTemporary(mipTextures[i]);
        }
    }
    
    public void BlitToMip(RenderTexture source, RenderTexture target, int mipLevel=0)
    {
